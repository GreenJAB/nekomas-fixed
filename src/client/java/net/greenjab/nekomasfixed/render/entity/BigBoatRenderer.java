...
public class BigBoatRenderer<T extends BigBoat, S extends BigBoatRenderState, M extends BigBoatModel<S>> extends EntityRenderer<T, S> {
	...
	public M getThisModel(EntityRendererProvider.Context context, ModelLayerLocation layer) {
		return (M)new BigBoatModel<S>(context.bakeLayer(layer));
	}
	...
	public void renderBanners(BigBoatRenderState bigBoatRenderState, PoseStack matrixStack, SubmitNodeCollector orderedRenderCommandQueue) {
		...
		bigBoatRenderState.bannerRenderState.submit(matrixStack, orderedRenderCommandQueue,
				bigBoatRenderState.lightCoords, OverlayTexture.NO_OVERLAY, bigBoatRenderState.outlineColor);
	}
	...
	public @NonNull S createRenderState() {
		return (S) new BigBoatRenderState();
	}
