    ...
    public static void registerLootTableAdds() {
        ...
        LootTableEvents.MODIFY.register((key, tableBuilder, _, holder) -> {
            if (key == BuiltInLootTables.CHARGED_CREEPER) {
                ...
                Holder<LootTable> lootTableHolder = holder.lookupOrThrow(Registries.LOOT_TABLE).getOrThrow(LootTableRegistry.SUPER_CHARGED_CREEPER_ENDERMAN_LOOT_TABLE);
                LootPool.Builder poolBuilder = LootPool.lootPool().add(NestedLootTable.lootTableReference(lootTableHolder).when(predicate));
                ...
                tableBuilder.pool(LootPool.lootPool().add(LootItem.lootTableItem(Items.BOOK).apply(EnchantRandomlyFunction.randomApplicableEnchantment(holder.lookupOrThrow(Registries.ENCHANTMENT)).withEnchantment(holder.lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(EnchantmentRegistry.LEECHING)))).add(LootItem.lootTableItem(Items.AIR)).build());
            }
        });
    }
}